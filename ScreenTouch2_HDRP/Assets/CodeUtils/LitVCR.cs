...
public class LitVCR : MonoBehaviour
{
    ...
    public void OnVideoSeekSlider(float seekvalue)
    {
        if (PlayingPlayer)
        {
            PlayingPlayer.Control.Seek(seekvalue * PlayingPlayer.Info.GetDuration());
        }
    }
    ...
    public string GetPlayInfo()
    {
        ...
        if (videoPaths.Count > 0)
        {
            ...
            else
            {
                if (PlayingPlayer)
                {
                    double time = PlayingPlayer.Control.GetCurrentTime();
                    double duration = PlayingPlayer.Info.GetDuration();
                    //视频播放时间ms，视频总时长ms，视频 index，文件名称
                    playinginfodto = $"{time},{duration},{videoindex},{Path.GetFileName(videoPaths[videoindex])}";
                }
            }
    ...
    {
        if (PlayingPlayer.MediaOpened)
        {
            PlayingPlayer.CloseMedia();
        }
        if (LoadingPlayer.MediaOpened)
        {
            LoadingPlayer.CloseMedia();
        }
    }
    ...
    public void SetLoopMode(LoopMode loopMode)
    {
        ...
        {
            PlayingPlayer.Loop = (loopMode == LoopMode.one);
        }
        if (LoadingPlayer != null)
        {
            LoadingPlayer.Loop = (loopMode == LoopMode.one);
        }
    }
    ...
    public void OpenVideoByFileName(String filename)
    {
        OpenVideoByFileName(filename, true);
    }
    public void OpenVideoByFileName(String filename, bool imgstopped)
    {
        int index = 0;
